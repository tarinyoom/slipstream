...
struct OwnedState {
    ...
    OwnedState(int nx, int ny, int nz, int n_emitters, bool ws) {
        ...
        const std::size_t sz = required_state_bytes(dims_span, n_emitters, ws);
        buf = host_alloc(sz);
        s = {};
        init_state(s, buf, sz, dims_span, n_emitters, ws);
    }
...
void project_3d(State& s) {
    ...
    k_cpu_3d::compute_projection(s.nx, s.ny, s.nz, s.obstacle, vx, vy, vz,
