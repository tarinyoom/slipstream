...
void set_emitter_block(PersistentState& s, int i_lo, int i_hi, int j_lo, int j_hi) {
...
void run(PersistentState& s, ScratchState& sc, int steps) {
...
TEST(Plume, ZeroBuoyancy_CentroidStable) {
    ...
    run(s, *arena.scratch, 1);
    auto p1 = plume_stats(s.density, N, N);
    run(s, *arena.scratch, 29);
...
TEST(Plume, Rises_CentroidIncreases) {
    ...
    run(s, *arena.scratch, 10);
    auto p10 = plume_stats(s.density, N, N);
    run(s, *arena.scratch, 30);
...
TEST(Plume, Rises_SkewnessShifts) {
    ...
    run(s, *arena.scratch, 5);
    auto p5 = plume_stats(s.density, N, N);
    run(s, *arena.scratch, 35);
    auto p40 = plume_stats(s.density, N, N);
